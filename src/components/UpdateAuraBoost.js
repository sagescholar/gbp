...
export const updateAuraBoost = (
  props_parent_state_summons_list,
  parent_state_list_aura_boost,
  parentSetStateUpdateAura,
  number
) => {
  /* SUMMONS_NAMEから属性を取り出す媒介変数 */
  const parent_state_summons_list = props_parent_state_summons_list[number]
  let content = cloneDeep(parent_state_list_aura_boost)
  ...
  Object.keys(parent_state_summons_list).map((key1_ /*MAIN or FRIEND*/) =>
    Object.keys(parent_state_summons_list[key1_]).map(
      (key2_ /* OPTI or OMEGA or SIXDRAGON */) =>
        Object.keys(parent_state_summons_list[key1_][key2_]).map(
          (key3_ /* SUMMONS NAME */) =>
            Object.keys(parent_state_summons_list[key1_][key2_][key3_]).map(
              (key4_) => {
                /* BOOST VALUE */
                let AuraType = cvt_summontype2opti[key2_];
                let AuraElement = cvt_summonname2element[key3_];
                if (
                  parent_state_summons_list[key1_][key2_][key3_][key4_].toggle
                )
                  rsl_list_aura_boost[AuraType][AuraElement] +=
                    parent_state_summons_list[key1_][key2_][key3_][key4_]
                      .value / 100.0;
              }
            )
        )
  ...
  content[number] = rsl_list_aura_boost
  parentSetStateUpdateAura(content);
};
